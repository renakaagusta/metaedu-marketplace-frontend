...
export default class OwnershipRepository {
  ...
  public async getOwnershipList(data: GetOwnershipListPayload): Promise<ApiResponse<GetOwnershipListResponse> | Error | undefined> {
    try {
      const response = await api.get<GetOwnershipListResponse, AxiosResponse<ApiResponse<GetOwnershipListResponse>>>(`${API_URL}/v1/ownership?offset=${data.offset}&limit=${data.limit}&keyword=${data.keyword}&order_by=${data.orderBy}&order_option=${data.orderOption}&${data.userId ? `user_id=${data.userId}` : `token_id=${data.tokenId}`}`)
  ...
  public async updateOwnership(data: UpdateOwnershipPayload): Promise<ApiResponse<undefined> | Error | undefined> {
    try {
      ...
      const response = await api.put<undefined, AxiosResponse<ApiResponse<undefined>>>(`${API_URL}/v1/ownership/${data.ownership.id}`, form, {
        headers: {
