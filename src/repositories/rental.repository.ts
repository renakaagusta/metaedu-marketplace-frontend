...
export default class RentalRepository {
  ...
  public async getRentalList(data: GetRentalListPayload): Promise<ApiResponse<GetRentalListResponse> | Error | undefined> {
    try {
      const response = await api.get<GetRentalListResponse, AxiosResponse<ApiResponse<GetRentalListResponse>>>(`${API_URL}/v1/rental?offset=${data.offset}&limit=${data.limit}${data.keyword ? `&keyword=${data.keyword}` : ''
        }&order_by=${data.orderBy}&order_option=${data.orderOption}${data.userId ? `&user_id=${data.userId}` : ``}${data.ownerId ? `&owner_id=${data.ownerId}` : ``}`)
