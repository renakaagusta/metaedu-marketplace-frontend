...
export default function MTokenCategoryCard(props: MTokenCategoryCard) {
  ...
  return (
    <ALink href={`/token?category=${tokenCategory.id}`}>
      <ACard
        hoverable
        className={'border-2 rounded-xl m-2 ' + clsx(theme === 'dark' ? 'bg-zinc-900 border-transparent' : 'bg-white border-grey-50', textColor)}
        cover={<AImage alt={tokenCategory.title} src={tokenCategory.icon} preview={false} height={200} style={{ borderTopRightRadius: 10, borderTopLeftRadius: 10 }} />}
        bodyStyle={{ padding: 18, paddingTop: 10 }}
      >
        <ALayout className={'m-0 p-0 ' + clsx(theme === 'dark' ? 'bg-zinc-900' : 'bg-white')}>
          <AText className={'font-bold ' + clsx(textColor)}>{tokenCategory.title}</AText>
        </ALayout>
      </ACard>
    </ALink>
  )
